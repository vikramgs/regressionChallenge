...
## <span style="color:brown; font-style:italic;">Introduction – to the challenge</span>
...
However, researchers typically do **not** observe true physiological Stress. Instead, they observe a survey‑based proxy:

$$
\text{StressSurvey} = g(\text{Stress}) + \nu
$$

where \( g(\cdot) \) is **non‑linear** and \( \nu \) represents measurement noise.
...
## <span style="color:brown; font-style:italic;">Visual Overview of the True vs Observed System</span>
...
### <span style="color:brown; font-style:italic;">The Data Generation Process</span>
...
## <span style="color:brown; font-style:italic;">Question 1. Bivariate Regression Analysis with StressSurvey:</span> 
...
## <span style="color:brown; font-style:italic;">Question 2. Visualization of Bivariate Relationship:</span> 
...
## <span style="color:brown; font-style:italic;">Question 3. Bivariate Regression Analysis with Time:</span> 
...
## <span style="color:brown; font-style:italic;">Question 4. Visualization of Bivariate Relationship:</span> 
...
## <span style="color:brown; font-style:italic;">Question 5. Multiple Regression Analysis:</span> 
...
print(f"True Intercept (β₀): 0.0")
print(f"True Stress coefficient (β₁): 1.0")
print(f"True Time coefficient (β₂): 0.1")
print(f"\nEstimated vs True:")
...
## <span style="color:brown; font-style:italic;">Question 6. Multiple Regression Analysis:</span> 
...
## <span style="color:brown; font-style:italic;">Question 7. Model Comparison:</span> 
...
<span style="color:brown; font-style:italic; font-weight:bold; text-align:justify;">Answer:</span>
...
print(f"  StressSurvey: p-value = {model3.pvalues['StressSurvey']:.2e} → {'Statistically Significant' if model3.pvalues['StressSurvey'] < 0.05 else 'Not Significant'}")
print(f"  Time: p-value = {model3.pvalues['Time']:.2e} → {'Statistically Significant' if model3.pvalues['Time'] < 0.05 else 'Not Significant'}")

print(f"\nModel 2 (Stress + Time):")
print(f"  Stress: p-value = {model4.pvalues['Stress']:.2e} → {'Statistically Significant' if model4.pvalues['Stress'] < 0.05 else 'Not Significant'}")
print(f"  Time: p-value = {model4.pvalues['Time']:.2e} → {'Statistically Significant' if model4.pvalues['Time'] < 0.05 else 'Not Significant'}")
...
- ❌ Completely wrong (wrong sign, wrong magnitude)
...
## <span style="color:brown; font-style:italic;">Question 8. Reflect on Real-World Implications:</span> 
...
## <span style="color:brown; font-style:italic;">Question 9. Avoiding Misleading Statistical Significance:</span> 
...
fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(6.5, 3), constrained_layout=True)

# Plot 1: StressSurvey vs Stress (full data)
ax1.plot(observDF['Stress'], observDF['StressSurvey'], 
         'o-', linewidth=1.5, markersize=5, color='purple')
ax1.set_xlabel('Actual Stress Level', fontsize=9)
ax1.set_ylabel('Stress Survey Response', fontsize=9)
ax1.set_title('Full Data: StressSurvey vs Stress', fontsize=10, fontweight='bold')
...
ax2.plot(subset_data['Stress'], subset_data['StressSurvey'], 
         'o-', linewidth=1.5, markersize=5, color='darkgreen')
ax2.set_xlabel('Actual Stress Level', fontsize=9)
ax2.set_ylabel('Stress Survey Response', fontsize=9)
ax2.set_title('Subset (Stress ≤ 2): More Linear', fontsize=10, fontweight='bold')
...
print("=" * 60)
print("I chose observations where Stress ≤ 2 because:")
print("1. In this range, StressSurvey = 3 × Stress (perfectly linear)")
print("2. The relationship StressSurvey-Stress is linear: 0→0, 1→3, 2→6")
...
1. **Linear Relationship:** In this range, StressSurvey = 3 × Stress, which is perfectly linear (0→0, 1→3, 2→6).
...
2. **StressSurvey Coefficient:** The coefficient is 0.33 instead of 1.0, but this makes sense because in this subset, StressSurvey = 3 × Stress. So if the true relationship is Anxiety = Stress + 0.1×Time, and StressSurvey = 3×Stress, then Anxiety = (1/3)×StressSurvey + 0.1×Time. The coefficient 0.33 = 1/3, which is exactly what we'd expect!
...
- StressSurvey = 3 × Stress (linear transformation)
- The relationship Anxiety = Stress + 0.1×Time can be rewritten as Anxiety = (1/3)×StressSurvey + 0.1×Time
...
### <span style="color:brown; font-style:italic;">Key Takeaways</span>

1. <span style="color:maroon; font-style:italic;">**Proxy variables can mislead:**</span> Even "good" proxies (like StressSurvey) can produce wrong coefficients if their relationship with the outcome is non-linear. In our analysis, StressSurvey produced a Time coefficient of -2.7799 (significant, p < 0.05), while the true coefficient is +0.1—a complete sign reversal.

2. <span style="color:maroon; font-style:italic;">**Statistical significance ≠ correctness:**</span> Both models showed statistical significance, but one had the wrong sign for the Time coefficient. Significance alone cannot distinguish correct from incorrect models when linearity is violated.

3. <span style="color:maroon; font-style:italic;">**High R-squared ≠ truth:**</span> A model can fit the data well (R² = 0.9350) while telling a completely false story. Model fit does not guarantee accurate coefficient estimates or correct causal interpretations.

4. <span style="color:maroon; font-style:italic;">**Regime analysis helps:**</span> Splitting the sample into subsets with more linear relationships can recover correct coefficients. By analyzing the subset where Stress ≤ 2, we recovered the correct Time coefficient (0.10), matching the true parameter value.

5. <span style="color:maroon; font-style:italic;">**Visual diagnostics are essential:**</span> Graphical analysis and regime identification are critical for detecting non-linearity and preventing misleading regression results. Always plot relationships before trusting regression outputs.
...
### <span style="color:brown; font-style:italic;">Final Assessment</span>
