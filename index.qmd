...
This analysis demonstrates a critical danger in regression analysis: **statistically significant results with high R-squared can be completely wrong** when relationships are non-linear or when proxy variables are used.

**Key Takeaways:**

1. **Proxy variables can mislead:** Even "good" proxies (like StressSurvey) can produce wrong coefficients if their relationship with the outcome is non-linear.

2. **Statistical significance ≠ correctness:** Both models showed statistical significance, but one had the wrong sign for the Time coefficient.

3. **High R-squared ≠ truth:** A model can fit the data well (R² = 0.9350) while telling a completely false story.

4. **Regime analysis helps:** Splitting the sample into subsets with more linear relationships can recover correct coefficients.

5. **Visual diagnostics are essential:** Always plot relationships before trusting regression results.
